...
const log = std.log.scoped(.lmdb_backend);
...
fn freshTestDir() []const u8 {
    const path = "/tmp/identityd-test-lmdb";
...
test "LmdbBackend: put and get" {
    ...
    try db.put("identities", "alice", "data_alice");
    try db.put("identities", "bob", "data_bob");

    const val = try db.get(std.testing.allocator, "identities", "alice");
    defer if (val) |v| std.testing.allocator.free(v);
    try std.testing.expectEqualStrings("data_alice", val.?);

    const missing = try db.get(std.testing.allocator, "identities", "charlie");
...
test "LmdbBackend: delete" {
    ...
    try db.put("identities", "alice", "data");
    try db.delete("identities", "alice");

    const val = try db.get(std.testing.allocator, "identities", "alice");
    try std.testing.expect(val == null);

    try db.delete("identities", "nonexistent");
}
...
test "LmdbBackend: overwrite value" {
    ...
    try db.put("identities", "alice", "old");
    try db.put("identities", "alice", "new");
...
test "LmdbBackend: separate namespaces" {
    ...
    try db.put("identities", "alice", "identity_data");
    try db.put("groups", "alice", "group_data");

    const u = try db.get(std.testing.allocator, "identities", "alice");
    defer if (u) |v| std.testing.allocator.free(v);
    try std.testing.expectEqualStrings("identity_data", u.?);

    const r = try db.get(std.testing.allocator, "groups", "alice");
    defer if (r) |v| std.testing.allocator.free(v);
    try std.testing.expectEqualStrings("group_data", r.?);
}
...
test "LmdbBackend: iterator prefix-bounded" {
    ...
    try db.put("edges", "alice\x00member_of\x00admins", "1");
    try db.put("edges", "alice\x00member_of\x00users", "1");
    try db.put("edges", "bob\x00member_of\x00users", "1");

    var iter = try db.iterator("edges", "alice\x00");
    defer iter.deinit();
...
test "LmdbBackend: writeBatch commit" {
    ...
    try batch.put("identities", "alice", "a");
    try batch.put("identities", "bob", "b");
    try batch.commit();

    const a = try db.get(std.testing.allocator, "identities", "alice");
    defer if (a) |v| std.testing.allocator.free(v);
    try std.testing.expectEqualStrings("a", a.?);

    const b_ = try db.get(std.testing.allocator, "identities", "bob");
    defer if (b_) |v| std.testing.allocator.free(v);
    try std.testing.expectEqualStrings("b", b_.?);
}
